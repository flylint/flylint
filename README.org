...
- docker
  Below snippet for Arch. Please see [[https://docs.docker.com/][document]] for another distribution
  #+begin_src shell
    sudo pacman -S docker
    sudo systemctl start docker.service
    sudo systemctl enable docker.service
    sudo gpasswd -a $USER docker
    sudo reboot                     # to reflect `gpasswd`
  #+end_src

- docker-compose
  Below snippet for Arch. Please see [[https://docs.docker.com/compose/install/][document]] for another distribution
  #+begin_src shell
    sudo pacman -S docker-compose
  #+end_src

Note: I'm using below versions.
#+begin_src shell
  $ cask --version
  0.8.4
  ...
  docker-compose version 1.24.1, build unknow
#+end_src
